from audio_processing.file_processing.mp3_audio_processing import stream_mp3_bytes_from_file
from audio_processing.file_processing.wav_audio_processing import stream_wav_bytes_from_file
from audio_processing.file_processing.mp3_audio_processing import get_mp3_file_features
...
class AudioFileProcessingFactory:
    ...
    def get_audio_byte_stream_from_file(cls, file):
        ...
        """
        audio_format = cls._extract_audio_format_if_supported(file)
    ...
    def get_audio_file_features(cls, file):
        audio_format = cls._extract_audio_format_if_supported(file)
        if audio_format == cls.SUPPORTED_FORMATS[0]:
            return get_mp3_file_features(file)
        elif audio_format == cls.SUPPORTED_FORMATS[1]:
            return get_wav_file_features(file)

    @classmethod
    def _extract_audio_format_if_supported(cls, file):
